...
def render_kpi_charts(summary_df: pd.DataFrame):
    ...
    for col, (ycol, ylabel, color) in pairs:
        ...
        ax.spines[:].set_color((1, 1, 1, 0.12))
            ...
            if imgs:
                ...
                for i, img in enumerate(imgs):
                    (c1 if i % 2 == 0 else c2).image(str(img),
                        caption=img.stem.replace("_", " "), width="stretch")
    else:
        ...
        if daily_df is not None:
            ...
            if MATPLOTLIB_AVAILABLE:
                ...
                if first_key:
                    ...
                    ax2.spines[:].set_color((1, 1, 1, 0.12))
                    plt.tight_layout()
                    st.pyplot(fig2)
                    plt.close(fig2)
    else:
    ...
    if result is None:
        ...
        if not zone_csvs and zone_df is not None:
            ...
            if daily is not None and MATPLOTLIB_AVAILABLE:
                ...
                ax_z.spines[:].set_color((1, 1, 1, 0.12))
                plt.tight_layout()
                st.pyplot(fig_z)
                plt.close(fig_z)
        else:
    ...
    if daily_df is None:
        ...
        if len(val_table) > 1 and numeric_cols:
            ...
            if MATPLOTLIB_AVAILABLE:
                ...
                ax_v.spines[:].set_color((1, 1, 1, 0.12))
                plt.tight_layout()
                st.pyplot(fig_v)
                plt.close(fig_v)
